...
(function() {
    ...
    if (!window.requestAnimationFrame) window.requestAnimationFrame = function(fn) {
...
(function($) {
    ...
    FlowGrid.prototype = {
        ...
        _getFixedEntryHeight: function($entry) {
            ...
            var size = {
                w: $thumb.data('original-width'),
                h: $thumb.data('original-height')
            };
...
(function($) {
    ...
    Progress.prototype.progress = function(val, callback) {
        ...
        if (this.options.autoComplate && this.p == this.options.max) {
            ...
            this.$bar.stop(true, false).animate({
                width: (this.p / this.options.max * 100) + '%'
            }, this.options.time, $.easing.def);
    ...
    Progress.prototype.complete = function(callback) {
        ...
        this.$bar.stop(true, false).animate({
            width: '100%'
        }, this.options.time, $.easing.def).promise().done(function() {
    ...
    function pageChange() {
        ...
        $(document).on('scroll.page.bq mousewheel.page.bq', function() {
            return false;
        })
    ...
    function _scrollToTop() {
        if ($(window).scrollTop() > 0) {
            return $(/(webkit)[ \/]([\w.]+)/i.test(navigator.userAgent) ? "body" : "html").delay(500).animate({
                scrollTop: 0
            }, 500).promise();
        } else {
            return wait(500);
